...
export interface IntroAnimationProps {
    /** Big line shown while the cards form a circle */
    title?: string;
    /** Small caps hint under the title */
    hint?: string;
    /** Heading that fades in once the cards morph into the arc */
    heading?: string;
    ...
    onComplete?: () => void;
    /** How long the formed arc stays on screen before `onComplete` fires */
    holdMs?: number;
}
...
export default function IntroAnimation({
    ...
    description = "A curated collection of projects by Aileen Romero. Scroll to browse, hover a card to flip it.",
    ...
    // 2. Scroll Rotation (Shuffling): rotates the bottom arc after the morph (> 600)
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // Normalize -1 to 1, then move +/- 100px
    ...
    // --- Completion ---
    // Fire onComplete once the arc has settled; scrolling back up cancels the hold
    const arcFormed = morphValue > 0.98;

    useEffect(() => {
        if (!arcFormed || !onComplete) return;
        const timer = setTimeout(onComplete, holdMs);
        return () => clearTimeout(timer);
    }, [arcFormed, onComplete, holdMs]);
                ...
                <div className="relative flex items-center justify-center w-full h-full">
                    {images.slice(0, TOTAL_IMAGES).map((src, i) => {
                        ...
                        if (introPhase === "scatter") {
                            ...
                            // B. Bottom Arc Position ("rainbow" arch, center is highest point)
                            ...
                            // Scroll rotation (0-360) as 0-1 progress, bounded so the
                            // last card stays in view
